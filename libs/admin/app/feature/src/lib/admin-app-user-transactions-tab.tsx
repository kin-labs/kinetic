...
import { UserTransactionListInput, useUserTransactionsQuery } from '@kin-kinetic/shared/util/admin-sdk'
import React, { useEffect, useState } from 'react'

export function AdminAppUserTransactionsTab({ appId, appEnvId }: { appId: string; appEnvId: string }) {
  const toast = useToast()
  const [input, setInput] = useState<UserTransactionListInput>({})
  const [{ data, error, fetching }, refresh] = useUserTransactionsQuery({ variables: { appId, appEnvId, input } })
